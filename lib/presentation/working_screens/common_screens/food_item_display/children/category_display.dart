...
class _CategoryDisplayState extends State<CategoryDisplay> {
  ...
  @override
  void initState() {
    context.read<MainBloc>()..add(CategoryLoadingEvent())..add(ProductForACategoryLoadingEvent(categoryId: 1));
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<MainBloc, MainState>(
      ...
      buildWhen: (prev, cur) {
        if (cur.runtimeType == CategoryLoadingSuccessState || cur.runtimeType == CategoryClickedState) {
          return true;
        } else {
          return false;
        }
                ...
                child: TextButton(
                  style: TextButton.styleFrom(
                   // backgroundColor: Colors.grey.shade100
                    foregroundColor: (clickedCategory == categories[index].categoryId) ? Colors.grey.shade700 : Colors.cyan
                  ),
                  onPressed: () {
                    context.read<MainBloc>().add(ProductForACategoryLoadingEvent(categoryId: categories[index].categoryId));
                  },
